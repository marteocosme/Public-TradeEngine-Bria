...
The Money Management module defines how trade risk, position size, and exposure are calculated for each eligible trade.
Its objective is to ensure consistent, bounded, and repeatable risk control, independent of entry strategy logic or indicator behavior.

This specification governs how much to trade, not when to trade.

## Inputs
The Money Management system requires the following inputs:
- Account Capital
    - Current account balance (or equity, as configured)
- Risk Configuration
    - Risk percentage per trade (e.g., 2%)
    - Maximum allowed concurrent risk
- Stop Loss Definition
    - Stop loss distance derived from ATR or strategy‑provided value
- Symbol Properties
    - Tick size
    - Tick value
    - Minimum lot size
...
## Outputs
The Money Management system produces the following outputs for a given trade decision:

- Calculated Lot Size
    - Normalized to broker constraints
- Risk Amount
    - Absolute monetary risk for the trade
- Stop Loss Price / Distance
    - Expressed in price units or points
- Trade Validity Flag
    - Indicates whether the trade is permitted under risk constraints

These outputs are consumed by the trade execution layer.

## Invariants
The following rules must always hold true:

- The risked amount must never exceed the configured risk percentage of account capital
- Lot size must respect broker constraints (min, max, step)
- Trades with invalid or zero stop‑loss distance must be rejected
- Risk calculations must not depend on strategy type or indicator selection
- The same inputs must always produce the same outputs
- Money Management does not decide trade direction or timing

These invariants ensure deterministic behavior and safe operation across all strategies.
