...
- Allows second position to run freely

### 📊 UML Sequence (Scaling Out)
...
sequenceDiagram
    ...
    Market->>Order1: Price reaches +1×ATR
    Order1-->>Market: Close position

    Market->>Order2: Adjust SL to Break Even
```

## 🛡️ 7. Break-Even Mechanism
### 📌 Trigger
...
### 📊 UML Activity (Trailing Stop)

```mermaid
flowchart TD
    A[Order #2 Active] --> B{Profit >= 2×ATR?}
    B -->|No| C[Hold Position]
    B -->|Yes| D[Start Trailing Stop]

    D --> E[Set SL = Price - 1.5×ATR]
    E --> F{Price Moves +0.5×ATR?}
    F -->|Yes| G[Adjust SL]
    F -->|No| E

    G --> H[Repeat Until Exit]
```
## 🚪 9. Exit Integration
### Exit Conditions (applies to runner)
1. Exit indicator signal  
2. Baseline cross  
3. Opposite C1 signal  
4. Stop-loss hit

### 📌 Interaction with Money Management 
- Trailing stop works **in parallel with exit signals**   
- Whichever triggers first → **trade closes** 
...
- Trailing stop must be **state-aware** 

## 🏗️ 11. UML Class Design (Architecture)
...
classDiagram
    ...
    TradeManager --> ATR
    TradeManager --> ExitManager
```


## 🎯 12. Key Takeaways  
- NNFX money management is built on **3 phases**: **
### 🧱 Phase 1 — Risk Control
- Fixed 2% risk  
- ATR-based SL

### ✂️ Phase 2 — Profit Securing
- Scale out at **1×ATR**   
- Move to break-even

### 🚀 Phase 3 — Profit Maximization
- Trail stop after **2×ATR**   
- Let runner capture trend

### 🚀 Final Insight (For Your EA
To implement this correctly in your system:

- 👉 You must track **trade state transitions

```
        OPEN → PARTIAL CLOSED → BREAK EVEN → TRAILING → EXIT
```
- 👉 This is **NOT just math — it’s a state machine**
